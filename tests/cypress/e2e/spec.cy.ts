...
describe('e2e tests', () => {
  ...
  const inputDatasourceSelector = 'input[id="data-source-picker"]';
  ...
  const inputSiteSelector = 'input[id="input_Site"]';
  ...
  describe('CRE tests', () => {
    it('time-usage panel by service (single host)', {}, () => {
      ...
      cy.get(inputDatasourceSelector).type('Checkmk ' + CmkCRE + '{enter}');
      cy.contains('Checkmk ' + CmkCRE).should('be.visible');

      cy.get(inputSiteSelector).type('{enter}'); // Site -> All Sites (first entry)
